(function() {
    ...
    const GameState = { MENU: 'menu', PLAYING: 'playing', GAMEOVER: 'gameover', SKINS: 'skins' };
    ...
    const player = {
        x: 0, y: 0, vx: 0, vy: 0,
        width: 30, height: 40,
        grounded: false, coyoteTime: 0, jumpBuffer: 0, jumpHeld: false,
        squash: 1, stretch: 1, facingRight: true, trail: [], maxTrail: 8
    };
    ...
    const input = { left: false, right: false, jump: false, jumpPressed: false, touchX: 0 };
    let inputCooldown = { left: 0, right: 0, jump: 0 };
    
    let touchStartX = 0, touchCurrentX = 0, jumpButtonTouchId = null;
    ...
    canvas.addEventListener('touchstart', (e) => {
        ...
        for (let touch of e.changedTouches) {
            ...
            if (x >= jumpRect.left && x <= jumpRect.right && y >= jumpRect.top && y <= jumpRect.bottom) {
    ...
    canvas.addEventListener('touchmove', (e) => {
        e.preventDefault();
        for (let touch of e.changedTouches) {
            if (touch.identifier === jumpButtonTouchId) continue;
            touchCurrentX = touch.clientX;
            input.touchX = (touchCurrentX - touchStartX) / 50;
            input.touchX = Math.max(-1, Math.min(1, input.touchX));
        }
    }, { passive: false });
    ...
    canvas.addEventListener('touchend', (e) => {
        e.preventDefault();
        for (let touch of e.changedTouches) {
            if (touch.identifier === jumpButtonTouchId) {
    ...
    function generatePlatform(y) {
        ...
        const platform = { x, y, width, height: PLATFORM_HEIGHT, type: 'normal', color: biome.platforms };
    ...
    function spawnTrap(type, x, y) {
        switch(type) {
            case 'moving':
                traps.push({ type: 'moving', x, y, width: 100, height: 15, vx: (Math.random() > 0.5 ? 1 : -1) * (2 + Math.random() * 2), minX: 50, maxX: W - 150, color: '#ff6b6b' });
                break;
            case 'falling':
                traps.push({ type: 'falling', x, y, width: 80, height: 15, fallTimer: 0, falling: false, vy: 0, color: '#f97316' });
                break;
            case 'hammer':
                traps.push({ type: 'hammer', x, y: y - 60, width: 20, height: 60, angle: 0, speed: 0.05 + Math.random() * 0.05, color: '#a1a1aa' });
                break;
            case 'push':
                traps.push({ type: 'push', x, y: y - 25, width: 40, height: 25, vx: 0, pushDir: 1, pushTimer: 0, color: '#84cc16' });
                break;
            case 'laser':
                traps.push({ type: 'laser', x, y: y - 100, width: 5, height: 100, active: true, timer: 0, interval: 60 + Math.random() * 60, color: '#ef4444' });
                break;
            case 'ice':
                traps.push({ type: 'ice', x, y, width: 80 + Math.random() * 60, height: 12, color: '#67e8f9' });
                break;
            case 'bounce':
                traps.push({ type: 'bounce', x, y, width: 50, height: 15, bounceForce: -20, color: '#fbbf24' });
                break;
            case 'fake':
                traps.push({ type: 'fake', x, y, width: 60, height: 15, breakTimer: 0, broken: false, color: '#a78bfa' });
                break;
            case 'wind':
                traps.push({ type: 'wind', x, y, width: 100, height: 200, strength: 3 + Math.random() * 2, direction: Math.random() > 0.5 ? 1 : -1, color: '#22d3d3' });
                break;
            case 'gravity':
                traps.push({ type: 'gravity', x, y, width: 80, height: 150, strength: 0.3, color: '#6366f1' });
                break;
            case 'walls':
                traps.push({ type: 'walls', x: x < W / 2 ? 0 : W - 60, y: y - 100, width: 60, height: 150, vx: x < W / 2 ? 2 : -2, color: '#dc2626' });
                break;
            case 'swing':
                traps.push({ type: 'swing', x, y: y - 80, length: 80, angle: Math.random() * 0.5, angleVel: 0, color: '#f59e0b' });
                break;
        }
    }
    ...
    function spawnLandingDust(x, y) {
        for (let i = 0; i < 8; i++) {
            ...
            spawnParticle(x, y, Math.cos(angle) * speed, Math.sin(angle) * speed - 1, '#ffffff', 20 + Math.random() * 10, 3 + Math.random() * 3);
        }
    }
    
    function spawnJumpEffect(x, y) {
        for (let i = 0; i < 5; i++) {
            spawnParticle(x + (Math.random() - 0.5) * 20, y, (Math.random() - 0.5) * 3, Math.random() * 2, '#4ade80', 15, 2 + Math.random() * 2);
        }
    }
    ...
    function update() {
        ...
        if (input.touchX !== 0) {
            moveX += input.touchX > 0.2 ? 1 : input.touchX < -0.2 ? -1 : 0;
        }
        ...
        for (let t of traps) {
            ...
            if (t.type === 'hammer') {
                collision = rectCollision(player.x, player.y, player.width, player.height, t.x - t.width / 2, t.y, t.width, t.height);
                if (collision) {
            ...
            } else if (t.type === 'walls' || t.type === 'moving' || t.type === 'falling' || t.type === 'ice' || t.type === 'bounce' || t.type === 'fake' || t.type === 'push') {
    ...
    function die() {
        ...
        for (let i = 0; i < 20; i++) {
            spawnParticle(player.x + player.width / 2, player.y + player.height / 2, (Math.random() - 0.5) * 10, (Math.random() - 0.5) * 10, '#ff6b6b', 30, 4 + Math.random() * 4);
        }
